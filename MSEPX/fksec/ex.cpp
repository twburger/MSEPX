...
/*! \file ex.cpp
\brief contains the implementation of fksec::ex, the exception class. */
...
\return an fkstr with the string returned by FormatMessage()
\exception none
\author Perry Rapp \<prapp@smartronix.com\> */
fkstr ex::GetWin32Desc() const
{
	LPVOID lpMsgBuf = 0;

	if ( 0 == FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER |
		FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
		NULL, errWin32, MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ),
		(LPTSTR) &lpMsgBuf, 0, NULL ) )
		return fkstr( _T( "Oops! FormatMessage() failed" ) );

	fkstr desc = (TCHAR *)lpMsgBuf;
	LocalFree(lpMsgBuf);
	return desc;
}
...
fkostream &fksec::operator<<( fkostream &o, const ex& e )
{
	...
	for ( i = 0; i < e.GetHopCount(); ++ i )
		o << std::setw( 3 ) << i << _T( " " ) << std::setw( 4 ) <<
			e[i].line << _T( " " ) << e[i].file << std::endl <<
			_T( "         " ) << e[i].msg << std::endl;

	if ( e.GetErrWin32() )
		o << e.GetWin32Desc() << std::endl;
	return o;
}
