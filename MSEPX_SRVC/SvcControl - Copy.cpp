...
DWORD __stdcall DoStartSvc(LPCWSTR szSvcName, DWORD argc, LPCWSTR args[])
{
    ...
    // Attempt to start the service. Startup arguments go here
	// For our purposes the arguments are "pipe" readhandle writehandle
	// with the handles being string versions of the handle values
    if (!StartService(
            schService,   // handle to service 
